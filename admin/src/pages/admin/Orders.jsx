...
export function Orders() {
  ...
  useEffect(() => {
    apiGet('/admin/businesses')
      .then((rows) => setBusinesses(Array.isArray(rows) ? rows : []))
      .catch(() => {})
  }, [])
  ...
  async function setOrderStatus(id, next) {
    setSaving(true)
    try {
      const updated = await apiPatch(`/admin/orders/${id}`, { status: next })
  ...
  return (
    <div className="orders-page">
      <header className="orders-hero">
        <div>
          <div className="orders-kicker"><Icon name="package" size={15} />{t('ord_kicker')}</div>
          <h2>{t('ord_h')}</h2>
          <p>{t('ord_p_admin')}</p>
        </div>
      ...
      <div className="orders-toolbar">
        <div className="orders-search">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('ord_search')} />
          <button type="button" className="btn btn-o" onClick={() => { setLoading(true); load() }}>{t('ord_find')}</button>
        </div>
        <div className="field" style={{ margin: 0, minWidth: 220 }}>
          <select value={businessId} onChange={(e) => setBusinessId(e.target.value)}>
            <option value="">{t('ord_all_biz')}</option>
            {businesses.map((b) => (
              <option key={b.id} value={b.id}>{b.biz}</option>
            ))}
          </select>
        </div>
        <div className="orders-filters">
          {['all', 'new', 'confirmed', 'preparing', 'ready', 'delivered', 'cancelled'].map((s) => (
            <button key={s} type="button" className={`orders-pill${status === s ? ' on' : ''}`} onClick={() => setStatus(s)}>
      ...
      <div className={`orders-layout${active ? ' has-detail' : ''}`}>
        <div className="orders-list card">
          {loading && !orders.length && <div className="orders-empty">{t('ord_loading')}</div>}
          ...
          {orders.map((o) => (
            <button key={o.id} type="button" className={`orders-row${o.id === activeId ? ' on' : ''}`} onClick={() => setActiveId(o.id)}>
        ...
        <aside className={`orders-detail card${active ? '' : ' empty'}`}>
          {!active ? (
            <div className="orders-empty">{t('ord_pick')}</div>
          ) : (
            <>
              <div className="orders-detail-head">
                <div>
                  <h3>{active.customer_name || active.customer_handle}</h3>
                  <p>{active.business_name} · {formatWhen(active.created_at, isAr)}</p>
                </div>
                <span className={`orders-badge st-${active.status}`}>{t(STATUS_KEYS[active.status] || active.status)}</span>
              </div>
              <div className="orders-grid">
                <div><label>{t('ord_phone')}</label><div>{active.customer_phone || active.customer_handle || '—'}</div></div>
                <div><label>{t('ord_channel')}</label><div>{active.channel || 'whatsapp'}</div></div>
                <div><label>{t('ord_type')}</label><div>{active.order_type === 'appointment' ? t('ord_type_appt') : t('ord_type_order')}</div></div>
                <div><label>{t('ord_delivery')}</label><div>{active.delivery_type || '—'}</div></div>
              </div>
              {active.address ? <div className="orders-block"><label>{t('ord_address')}</label><p>{active.address}</p></div> : null}
              <div className="orders-block">
                <label>{t('ord_items')}</label>
              ...
              {active.notes ? <div className="orders-block"><label>{t('ord_notes')}</label><p style={{ whiteSpace: 'pre-wrap' }}>{active.notes}</p></div> : null}
              <div className="orders-total-row">
                <span>{t('ord_total_amt')}</span>
                <b>{money(active.total, active.currency) || '—'}</b>
              </div>
              <div className="orders-actions">
                <label>{t('ord_update_status')}</label>
                <div className="orders-status-btns">
                  {['new', 'confirmed', 'preparing', 'ready', 'delivered', 'cancelled'].map((s) => (
                    <button key={s} type="button" className={`btn ${active.status === s ? 'btn-g' : 'btn-o'}`} disabled={saving || active.status === s} onClick={() => setOrderStatus(active.id, s)}>
                      {t(STATUS_KEYS[s])}
                    </button>
                  ))}
                </div>
