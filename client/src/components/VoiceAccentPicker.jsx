...
/**
 * Accent / TTS voice picker with per-voice preview playback.
 * value = selected voice id; onChange(id) when user picks one.
 */
export function VoiceAccentPicker({
  value = 'alloy',
  onChange,
  previewPath = '/tts/preview',
  voicesPath = '/tts/voices',
  labels = {},
}) {
  const [voices, setVoices] = useState([])
  const [playing, setPlaying] = useState(null) // voice id loading/playing
  ...
  return (
    <div className="voice-accent">
      <div className="voice-accent-head">
        ...
        <span className="hint">{labels.hint || 'Preview each voice, then select the one WhatsApp voice notes will use.'}</span>
      </div>
