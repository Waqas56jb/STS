...
/**
 * Catches any render/runtime error in the tree and shows a recoverable
 * screen instead of a blank white page ("crash"). Inline styles only, so it
 * still renders if the stylesheet failed to load. Reload re-runs the app on
 * the SAME URL, so the user stays on the page they refreshed.
 */
    ...
    return (
      <div style={wrap}>
        <div style={card}>
          ...
          <p style={p}>The page hit an unexpected error. Reloading usually fixes it.</p>
          <div style={row}>
            <button style={btnPrimary} onClick={() => window.location.reload()}>Reload page</button>
            <button style={btnGhost} onClick={() => { window.location.href = '/' }}>Back to home</button>
          </div>
        </div>
      </div>
    )
