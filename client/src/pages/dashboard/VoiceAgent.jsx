...
export function DialCard({ dialEndpoint = '/me/calls/dial', onCalled }) {
  ...
  async function call() {
    const to = num.trim()
    if (to.replace(/[^\d]/g, '').length < 6) { toast(isAr ? 'أدخل رقماً صحيحاً' : 'Enter a valid number'); return }
    setBusy(true)
    try {
      await apiPostAuth(dialEndpoint, { to })
      toast(isAr ? 'جارٍ الاتصال… الوكيل سيتحدث معهم' : 'Calling… the agent will talk to them')
      setNum('')
      onCalled?.()
    } catch (e) {
      toast(e.message || (isAr ? 'فشل الاتصال' : 'Call failed'))
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        ...
        <button className="btn btn-o" onClick={load} title="Refresh"><Icon name="activity" size={14} /></button>
      </div>
      <div className="tbl">
        <table>
          <thead><tr>
      ...
      <div className="modal-card" style={{ maxWidth: 560, maxHeight: '86vh', overflow: 'auto' }}>
        ...
        {call.summary && (
          <div style={{ background: '#F0F9F6', border: '1px solid var(--line)', borderRadius: 10, padding: '10px 12px', fontSize: 13, marginBottom: 14 }}>
            <b>{isAr ? 'الملخص' : 'Summary'}:</b> {call.summary}
          </div>
        )}
        {turns.length === 0 ? (
          <div style={{ color: 'var(--mut)', fontSize: 13, padding: 8 }}>{isAr ? 'لا يوجد نص للمكالمة.' : 'No transcript captured.'}</div>
        ) : (
            ...
            {turns.map((tn, i) => (
              <div key={i} style={{ alignSelf: tn.role === 'agent' ? 'flex-start' : 'flex-end', maxWidth: '82%' }}>
                ...
                <div style={{ fontSize: 10.5, color: 'var(--mut)', marginTop: 2, textAlign: tn.role === 'agent' ? 'start' : 'end' }}>
                  {tn.role === 'agent' ? (isAr ? 'الوكيل' : 'Agent') : (isAr ? 'المتصل' : 'Caller')}
                  {tn.at ? ' · ' + new Date(tn.at).toLocaleTimeString(isAr ? 'ar' : 'en-GB') : ''}
                </div>
              </div>
            ))}
          </div>
